...
/// <reference types="bun" />
/**
 * Script pour générer email-theme.ts depuis global.css
 *
 * Ce script lit le fichier global.css, extrait les variables CSS OKLCH,
 * les convertit en hexadécimal, et génère le fichier email-theme.ts
 */
...
// Configuration
const GLOBAL_CSS_PATH = `${PROJECT_ROOT}apps/frontend/src/app/globals.css`;
const EMAIL_THEME_PATH = `${PROJECT_ROOT}packages/emails/template/email-theme.ts`;
...
/**
 * Génère le contenu du fichier email-theme.ts
 */
...
async function main() {
  ...
  // Lire le fichier CSS
  try {
    const cssContent = await Bun.file(GLOBAL_CSS_PATH).text();
  ...
  // Écrire le fichier
  await Bun.write(EMAIL_THEME_PATH, themeContent);
